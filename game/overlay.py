...
class Overlay:
    ...
    def handle_events(self, mouse: Coords, events: list[pygame.event.Event]) -> bool:
        ...
        if self.minimap_backdrop_rect.collidepoint(mouse):
            ...
            if self.selected_entity.name in actions_dictionary:  # TODO: is name in dictionary check necessary at the end?
                for n, action in enumerate(action for action in actions_dictionary[self.selected_entity.name] if action.available and actions.flags[action.required]):
    ...
    def generate_tooltips(self):
        ...
        self.tooltips["fungusfarm"] = (costly_tooltip("Farm", (100, 50, 0), "Turns collected leaves into food"), True)
        self.tooltips["lumbermill"] = (costly_tooltip("Lumbermill", (20, 50, 0), "Dropoff point for collected wood"), True)
        self.tooltips["foundry"] = (costly_tooltip("Foundry", (100, 50, 0), "Turns collected ore into metal"), True)
        self.tooltips["nexus"] = (costly_tooltip("Nexus", (0, 0, 0), "Creates workers and queens. You lose when your last Nexus is destroyed"), True)
        self.tooltips["barracks"] = (costly_tooltip("Barracks", (150, 200, 50), "Creates soldier ants"), True)
        self.tooltips["siegery"] = (costly_tooltip("Siegery", (200, 500, 100), "Creates stronger mechanical ants"), True)
        self.tooltips["tower"] = (costly_tooltip("Tower", (100, 50, 30), "Attacks nearby enemies"), True)

        self.tooltips["worker"] = (costly_tooltip("Worker", (0, 0, 30), "Collects resources and builds structures"), True)
        self.tooltips["queen"] = (costly_tooltip("Queen", (250, 250, 500), "Capable of creating a new Nexus"), True)
        self.tooltips["soldier"] = (costly_tooltip("Soldier", (0, 10, 40), "Basic foot soldier"), True)
        self.tooltips["phrag"] = (costly_tooltip("Phragmotist", (0, 30, 50), "Soldier with a shielded head, granting extra defence"), True)
        self.tooltips["alate"] = (costly_tooltip("Alate", (0, 300, 50), "Jet-powered, low-flying kamikaze that deals a ton of damage to buildings"), True)
        self.tooltips["major"] = (costly_tooltip("Major", (0, 100, 75), "Large, technologically enhanced soldier. Has trouble walking through forests"), True)

        self.tooltips["arrhenotoky"] = (costly_tooltip("Arrhenotoky (Research)", (100, 100, 300), "Unlocks the creation of queens"), True)
        self.tooltips["cybernetics"] = (costly_tooltip("Cybernetics (Research)", (200, 300, 50), "Unlocks the creation of the siegery"), True)
        self.tooltips["propulsion"] = (costly_tooltip("Jet Propulsion (Research)", (200, 400, 100), "Unlocks the creation of kamikaze alates"), True)
