...
def alter(color: Color, degree: float) -> Color:
    ...
    Does not change the alpha of a color.
    """
    return (*cap(round(c * degree) for c in color[:3]), color[3])


def lerp(col1: Color, col2: Color, degree: float) -> Color:
    """
    Interpolate between two given colors by a given degree
    """
    return tuple(int(_lerp(a, b, degree)) for a, b in zip(col1, col2))
